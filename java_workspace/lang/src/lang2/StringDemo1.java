...
public class StringDemo1 {
	...
	public static void main(String[] args) {
		...
		// boolean equals(Object other) : 자신과 other로 전달받은 문자열의 내용을 비교해서
		//								   그 결과를 boolean(true or false)값을 반환 	
		// boolean equalsIgnoreCase(String other) : 대소문자를 구분하지 않고 비교해서
		//											그 결과를 반환한다.
		...
		// 문자열도 배열이여서 0번째부터 시작한다.
		// String substring(int begin) : 지정된 위치에서부터 끝까지 포함된 새로운 문자열을 반환한다.
		// String substring(int begin, int end) : 지정된 범위내의 문자열이 포함된 
		//									         새로운 문자열을 반환한다
		...
		// -> 문자열에서 지정된 문자열(old)을 새문자열(new)로 변환한 새로운 문자열을 반환한다.
		// String replace(String regexp, String replacement)
		// -> 문자열에서 지정된 정규표현식과 일치하는 문자열을 찾아서 새 문자열로 변환한
		//    새로운 문자열을 반환한다.
		...
		// String[] split(String reqexp) : 문자열을 지정된 표현식(혹은 구분문자로) 자른
		//								    새로운 배열을 반환한다.
		//								   ,(콤마)를 기준으로 반환한다.
		...
		// boolean contain(CharSequence s) : 문자열에 지정된 문자열이 포함되어 있으면
		//									 true를 반환한다.
		// int indexOf(String str) : 문자열에 지정된 문자열이 있으면 그 위치를 반환한다.
		//							  없으면 -1을 반환한다.(왼쪽부터)
		// int lastIndexOf(String str) : 문자열에 지정된 문자열이 마지막으로 등장하는
		//								  위치를 반환한다. 없으면 -1을 반환한다.(오른쪽부터)
		...
		// boolean starstWith(String prefix) : 문자열이 지정된 문자열로 시작하면 true를 반환한다.
		...
		// int compareTo(String other) : 다른 문자열과 사전(dictionary)순서를 기준으로 비교했을때
		//								 같으면 0, 빠르면 음수, 늦으면 양수를 반환한다.
