...
<script type="text/javascript">
	...
	function openPopup(event) {
		...
		// onreadystatechange는 XHR의 readyState값이 변할 때 마다 발생하는 이벤트
 		xhr.onreadystatechange = function() {
			...
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				// 변환값이 배열이라면 for문을 돌려서 값을 꺼내고 객체일경우 name.value로 값을 꺼낸다.
	...
	function getEmployees() {
		...
		// * XHR객체의 onreadystatechange이벤트에, 이벤트 발생시 실행될 함수를 등록한다.
		// * XHR객체의 readyState값이 4면 응답을 받은 것이다.
		// * XHR객체의 status값이 200이면 성공적인 응답을 받은것이다.
		// * 서버가 보낸 응답데이터는 XHR객체의 responseText에 보관되어 있다.
		// * 서버가 보낸 응답데이터가 JSON 형식의 데이터라면 JSON.parse() 함수를 사용해서
		//   javascript의 배열이나 객체로 변환할 수 있다
		// * 배열이나 객체로 변환되었다면 자바스크립트 문법을 사용해서 값을 조회하고, 조회된 값을 이용해서
		//   HTML 컨텐츠를 만든 다음, 특정 태그에 부착시켜서 화면에 표시되게 한다.
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4 && xhr.status == 200) {
				var text = xhr.responseText;
